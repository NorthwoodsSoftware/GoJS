import{$ as e,C as t,G as n,H as r,L as i,T as a,W as o,c as s,et as c,m as l,st as u,x as d,z as f}from"../chunks/BY_RssUb.js";import"../chunks/DTR9fq_7.js";import"../chunks/xihTtKlq.js";import{n as p,t as m}from"../chunks/DzUjTTvJ.js";var h=u({load:()=>g,prerender:()=>!0}),g=()=>({metadata:{title:`Accessible Diagram Example`,titleShort:`ARIA Accessibility`,indexDescription:`Demonstrates a customized CommandHandler to update a live HTML element for accessibility.`,screenshot:`orgcharteditor`,priority:1,tags:[`commands`,`extensions`],description:`Demonstrates a customized CommandHandler to update a live HTML element for accessibility.`},htmlContent:`<div id="myDiagramDiv" style="border: solid 1px black; height: 625px"></div>\r
  Theme:\r
  ...
  <output id="myLiveRegion"></output>\r
  <div>\r
    Edit details:<br />\r
...
\r
      'commandHandler.liveElementId': 'myLiveRegion',\r
      // customize what is read by a screen reader\r
      'commandHandler.focusChanged': (oldobj, newobj, cmd) => {\r
        const region = document.getElementById(cmd.liveElementId);\r
        if (region) region.textContent = describe(oldobj, newobj, cmd);\r
      }\r
    });\r
\r
    function describe(oldobj, newobj, cmd) {\r
      let str = '';\r
      if (!newobj) {\r
        str = 'Organization Chart for the Example Company';  // when no focus GraphObject -- describe the whole Diagram\r
      } else if (newobj instanceof go.Link) {\r
        str = cmd.describe(oldobj, newobj, cmd);\r
      } else if (newobj instanceof go.Node) {\r
        const d = newobj.data;  // custom string including most important data\r
        str = \`Employee \${d.name} \${d.title} \${d.dept}\`;\r
        if (newobj.findNodesOutOf().count > 0) {\r
          let emps = '';\r
          newobj.findNodesOutOf().each(to => {\r
            if (emps) emps += ', ';\r
            emps += to.data.name;\r
          });\r
          if (emps) str += '; Employees: ' + emps;\r
        }\r
      } else {\r
        str = cmd.describe(oldobj, newobj, cmd)\r
        const tt = toolTipTextConverter(newobj);\r
        if (tt) str += ' ' + tt;\r
      }\r
      return str;\r
    }\r
...
\r
    function addEmployee(node) {\r
      ...
      myDiagram.commandHandler.focus = newnode;\r
    }\r
\r
...
\r
  window.addEventListener('keydown', enableKeyboard);`,cssCode:`:modal {\r
    padding: 2rem;\r
...
\r
  #hidden {\r
    ...
    This sample is a copy of the <a href="./orgChartEditor">Org Chart Editor</a> sample,\r
    but has better support for accessibility.  Diff the two files to see the enhancements.\r
  </p>\r
  <p>\r
    First, this sample sets <a>CommandHandler.isFocusEnabled</a> and <a>CommandHandler.isVirtualPointerEnabled</a> to true\r
    when the user tabs into the diagram,\r
    so that the user does not have to type <code>Ctrl-Alt-Enter</code>, which they will probably not know about.\r
  </p>\r
  <p>\r
    Second, this sample provides a custom <a>CommandHandler.focusChanged</a> event handler that updates a div named "myLiveRegion".\r
    As navigation focus changes within the diagram, the text of the live region is set to a description of the current\r
    <a>CommandHandler.focus</a> GraphObject.\r
    Furthermore, when a new employee node is added, it becomes the new focus.\r
  </p>\r
  <p>\r
    Each diagram should be customized thoroughly for your particular application, where you know what your users want or need to know.\r
    Note how in this sample it talks about employees instead of the more generic "Node"s and "Link"s in the\r
    <a href="./orgChartEditor">Org Chart Editor</a> sample.\r
    Also, this sample makes sure there is a textual description of the "TreeExpanderButton" and the button to call AddEmployee.\r
    In this case it just uses the same text that is used in a "ToolTip" for each button.\r
  </p>\r
  <p>\r
    For another sample demonstrating screen-reader assistance, see <a href="./IVRtree">IVR Tree</a> sample.\r
    A more sophisticated accessibility customization could provide information for where the user could change focus to, and how to do so.\r
  </p>`,descriptionCodeBlocks:[]}),_=a(`<meta name="description"/> <meta property="og:description"/>`,1),v=a(`<meta property="og:title"/> <!>`,1),y=a(`<!> <!>`,1);function b(a,u){c(u,!0);let h=[`commands`,`extensions`];var g=y();l(`jcrpfn`,e=>{var a=v(),c=o(a),l=n(c,2),p=e=>{var r=_(),i=o(r),a=n(i,2);f(()=>{s(i,`content`,u.data.metadata.description),s(a,`content`,u.data.metadata.description)}),t(e,r)};d(l,e=>{u.data.metadata.description&&e(p)}),f(()=>s(c,`content`,`${u.data.metadata.title??``} | GoJS Diagramming Library`)),i(()=>{r.title=`${u.data.metadata.title??``} | GoJS Diagramming Library`}),t(e,a)});var b=o(g);p(b,{get data(){return u.data}}),m(n(b,2),{get tags(){return h}}),t(a,g),e()}export{b as component,h as universal};