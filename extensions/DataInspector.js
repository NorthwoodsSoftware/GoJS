...
class Inspector {
     ...
     */
    static showIfPresent(part, propname) {
        if (part instanceof go.Part)
            part = part.data;
        return typeof part === 'object' && part[propname] !== undefined;
    }
    /**
    ...
    inspectObject(object) {
        ...
        if (inspectedObjects) {
            ...
            mainDiv.innerHTML = '';
            const shared = new Map(); // for properties that the nodes have in common
            const properties = new Map(); // for adding properties
            const all = new Map(); // used later to prevent changing properties when unneeded
