...
// import { goInstance as go } from '../../shared/gojs.service';
...
class DrawCommandHandler extends go.CommandHandler {
    ...
    copyToClipboard(coll) {
        try {
            ...
            this._lastPasteOffset.set(this.pasteOffset); // reset paste offset
        }
    }
    /**
    ...
    pasteFromClipboard() {
        ...
        try {
            ...
            if (clipstr === null || clipstr === '' || clipfrmt !== this.diagram.model.dataFormat) {
                ...
                const pastedParts = new go.Set().addAll(copymap.iteratorValues);
                // CUSTOM WMX FUNCTIONALITY
                pastedParts.each((part) => {
                    // add unique uuid for each pasted part
                    const uuid = UUID();
                    this.diagram.model.setKeyForNodeData(part.data, uuid);
                    part.data.uuid = uuid;
                });
                // paste at an offset
                this.diagram.moveParts(pastedParts, this._lastPasteOffset, false);
                this._lastPasteOffset.add(this.pasteOffset);
                // END CUSTOM WMX FUNCTIONALITY
                return pastedParts;
            }
        }
        catch (ex) {
