...
export class ColumnResizingTool extends go.Tool {
   ...
   */
  constructor(init?: Partial<ColumnResizingTool>) {
    super();
   ...
   */
  get adornedTable(): go.Panel | null {
      ...
      if (
        ...
        if (adornment !== null) {
          ...
          adornment.elements.each((h: go.GraphObject) => {
            ...
            h.alignment = new go.Spot(
              0,
              0,
              pad.left + coldef.position + wid + sep / 2,
              pad.top + h.height / 2
            );
          });
   ...
   */
  makeAdornment(table: go.Panel): go.Adornment {
   ...
   */
  makeHandle(table: go.Panel, coldef: go.RowColumnDefinition): go.GraphObject | null {
  ...
  resize(newPoint: go.Point): void {
    ...
    this.doResize(
      ...
      Math.max(
        0,
        locpt.x - pad.left - coldef.position - (coldef.total - coldef.actual) - sep / 2
      )
    );
  }
   ...
   */
  doResize(coldef: go.RowColumnDefinition, width: number) {
    coldef.width = width;
  }
   ...
   */
  computeResize(p: go.Point): go.Point {
    return p;
  }
