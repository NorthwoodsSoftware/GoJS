...
export class DrawCommandHandler extends go.CommandHandler {
  ...
  // support for writing and reading local files holding Model.toJson JSON-formatted text
  private _localFileType: string;
  private _localFileDropElement: HTMLElement | null;
  private _preventPropagation;
  private _handleDrop;
  private _localFileInput: HTMLInputElement | null;
  private _handleFileInputChange;
  private _loader: ((diagram: go.Diagram, model: go.Model, name: string) => void) | null;
  ...
  constructor(init?: Partial<DrawCommandHandler>) {
    ...
    this._preventPropagation = (e: Event) => {
      e.stopPropagation();
      e.preventDefault();
    };
    this._handleDrop = (e: DragEvent) => {
    ...
    this._handleFileInputChange = (e: Event) => {
      const files = this._localFileInput?.files;
      if (files && files.length > 0) {
        this.diagram.focus();
        this.loadLocalFile(files[0], this.loader);
      }
  ...
  pullToFront(): void {
    ...
    const layers = new Map<go.Layer, number>();
    diagram.selection.each((part) => {
  ...
  /** @hidden @internal */
  private saveFile(name: string, mimetype: string, contents: string): void {
  ...
  public saveLocalFile(options?: { name?: string, mimetype?: string }) {
    ...
    let type = options?.mimetype;
    if (!type) type = 'application/text';
        ...
        reader.onload = e => {
          if (typeof e.target?.result === 'string') {
            ...
            if (loader) {
              ...
              if (!newmodel.name) newmodel.name = name;
