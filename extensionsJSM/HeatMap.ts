...
export class HeatMap {
  private _diagram: go.Diagram | null;
  private _heatMapPart: go.Part;
  private _colors: Array<Array<number>>;
  private _updater: () => void;
  private _changer: (e: go.ChangedEvent) => void;
   ...
   * @param init
   */
  constructor(diag?: go.Diagram, init?: Partial<HeatMap>) {
    this._diagram = null;
    ...
    this._changer = (e) => {
      if (e.isTransactionFinished) this.updateHeatMap();
    };
  ...
  get diagram(): go.Diagram | null {
    return this._diagram;
  }
  set diagram(value: go.Diagram | null) {
  ...
  get heatMapPart(): go.Part {
    return this._heatMapPart;
  }
  private set heatMapPart(value: go.Part) {
    if (value !== this.heatMapPart) {
      if (this.diagram !== null) this.diagram.remove(this.heatMapPart);
      this._heatMapPart = value;
      if (this.diagram !== null) this.diagram.add(this.heatMapPart);
    }
  }
  ...
  get colors(): Array<Array<number>> {
    return this._colors;
  }
  set colors(value: Array<Array<number>>) {
    if (
      !Array.isArray(value) ||
      value.length < 2 ||
      !value.every(
        (a) => Array.isArray(a) && a.length === 4 && a.every((n) => typeof n === 'number')
      )
    ) {
      throw new Error(
        'HeatMap.colors must be an Array of Array of four numbers, not: ' + value
      );
    }
    this._colors = value;
    this.updateHeatMap();
   ...
   */
  getTemperature(part: go.Part): number {
    return 1;
  }
   ...
   * @see {@link normalizeTemperature}
   */
  computeStartingColorIndex(frac: number): number {
   ...
   */
  renderImageData(area: go.Rect, size: go.Size): ImageData | null {
  ...
  updateHeatMap(): void {
    ...
    const picture = this.heatMapPart.findObject('IMG') as go.Picture;
    picture.width = vb.width;
    picture.height = vb.height;
    picture.scale = diag.scale;
    const canvas = picture.element as HTMLCanvasElement;
    canvas.width = w;
    canvas.height = h;
  ...
  private _renderLink(
    ...
    if (part.pointsCount < 2) return Infinity;
    const b = part.routeBounds.copy();
    if (!b.intersectsRect(vb)) return Infinity;
    ...
    if (part.computeCurve() === go.Curve.Bezier) {
      ...
      for (let i = 1; i < part.pointsCount; i++) {
        ...
        const m =
          Math.abs(vq.x - vp.x) > Math.abs(vq.y - vp.y) ? vq.x - vp.x : vq.y - vp.y;
        const am = Math.abs(m);
  ...
  private _renderPart(
    ...
    let obj: go.GraphObject | null = part.selectionObject;
    if (
      obj instanceof go.Panel &&
