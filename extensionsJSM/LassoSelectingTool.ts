 ...
 */
export class LassoSelectingTool extends go.Tool {
  private _shape: go.Shape;
   ...
   */
  selectInShape(shp: go.Shape): void {
    const diagram = this.diagram;
    ...
    if (e.meta || e.control) {  // toggle or deselect
      ...
      while (sit.next()) {
        const sp = sit.value;
        if (!parts.has(sp)) tounselect.add(sp);
      }
      ...
      while (it.next()) {
        ...
        if (!ps.isSelected && shp.polygonContainsRect(temp)) ps.isSelected = true;
      }
    }
  }
}
