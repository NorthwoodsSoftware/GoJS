 ...
 * The source code for this is at extensionsJSM/PolygonDrawingTool.ts.
...
export class PolygonDrawingTool extends go.Tool {
  ...
  private _hasArcs: boolean;
  private _isOrthoOnly: boolean;
  private _isGridSnapEnabled: boolean;
   ...
   */
  constructor(init?: Partial<PolygonDrawingTool>) {
    super();
   ...
   */
  modifyPointForGrid(p: go.Point): go.Point {
    ...
    if (this.isOrthoOnly && segments.count > 0) {
      ...
      if (
        pregrid.distanceSquared(lastPt.x, pregrid.y) < pregrid.distanceSquared(pregrid.x, lastPt.y)
      ) {
        // closer to X coord
    ...
    if (this.archetypePartData !== null) {
      ...
      if (tempgeo !== null) {
        ...
        if (tempfig !== null && tempfig.segments.count >= (this.isPolygon ? 2 : 1)) {
          ...
          if (part) this.transactionResult = this.name;
        }
      }
    }
    this.stopTool();
   ...
   * @param geo 
   */
  makeShape(geo: go.Geometry): go.Part | null {
    const diagram = this.diagram;
