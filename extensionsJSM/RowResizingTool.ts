...
export class RowResizingTool extends go.Tool {
   ...
   */
  constructor(init?: Partial<RowResizingTool>) {
    super();
        ...
        if (adornment !== null) {
          ...
          adornment.elements.each((h) => {
            ...
            h.alignment = new go.Spot(
              ...
              0,
              pad.left + h.width / 2,
              pad.top + rowdef.position + hgt + sep / 2
            );
          });
   ...
   */
  makeHandle(table: go.Panel, rowdef: go.RowColumnDefinition): go.GraphObject | null {
   ...
   */
  doResize(coldef: go.RowColumnDefinition, height: number) {
