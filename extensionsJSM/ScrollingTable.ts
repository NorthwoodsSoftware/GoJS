...
go.GraphObject.defineBuilder('AutoRepeatButton', (args) => {
  ...
  const button = go.GraphObject.build('Button').attach({
...
go.GraphObject.defineBuilder('ScrollingTable', (args) => {
  ...
  function showScrollButtons(bar: go.Panel, show: boolean) {
    ...
    const down = bar.findObject('DOWN');
    if (down) down.opacity = show ? (idx < getMaxIdx(table) ? 1.0 : 0.3) : 0.0;
  ...
  return $(go.Panel, 'Table',
    ...
    $(go.RowColumnDefinition, { column: 1, sizing: go.Sizing.None }),
    $(go.Panel, 'Table',
      {
        name: 'SCROLLBAR',
        column: 1,
      ...
      $(go.RowColumnDefinition, { row: 2, sizing: go.Sizing.None })
    )
  );
});
