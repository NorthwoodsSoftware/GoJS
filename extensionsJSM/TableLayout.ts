 ...
 * The source code for this is at extensionsJSM/TableLayout.ts.
...
export class TableLayout extends go.Layout {
   ...
   * @returns this
   */
  addRowDefinition(row: number, init: Partial<go.RowColumnDefinition>): this {
   ...
   * @returns this
   */
  addColumnDefinition(col: number, init: Partial<go.RowColumnDefinition>): this {
   ...
   */
  override doLayout(coll: go.Iterable<go.Part>): void {
   ...
   * @virtual
   */
  protected beforeMeasure(
    parts: go.List<go.Part>,
    rowcol: Array<Array<Array<any>>>
  ): void {}
      ...
      if (isFinite(colHerald.width)) {
        ...
        if (isFinite(colleft) && (nosizeCols as any)[child.column] !== null) {
          if (desiredColTotal === 0) w = colHerald.actual + colleft;
            ...
            w =
              /*colHerald.actual +*/ ((nosizeCols as any)[child.column] /
                desiredColTotal) *
              originalcolleft;
          }
        } else {
