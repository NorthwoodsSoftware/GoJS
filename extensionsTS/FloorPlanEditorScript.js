...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../release/go", "./DrawCommandHandler", "./GuidedDraggingTool", "./ResizeMultipleTool", "./RotateMultipleTool"], factory);
    }
    ...
    */
    ...
    function init() {
        ...
        if (openDoc)
            openDoc.style.visibility = 'hidden';
        // hides remove HTML Element
        var removeDoc = document.getElementById('removeDocument');
        if (removeDoc)
            removeDoc.style.visibility = 'hidden';
        ...
        myDiagram =
            $(go.Diagram, 'myDiagramDiv', {
                ...
                'ExternalObjectsDropped': function (e) {
                    var elt = document.getElementById('myDiagramDiv');
                    if (elt)
                        elt.focus(); // assume keyboard focus should be on myDiagram
                    myDiagram.toolManager.draggingTool.clearGuidelines(); // remove any guidelines
                }
            });
        // sets the qualities of the tooltip
        var tooltiptemplate = $('ToolTip', $(go.TextBlock, { margin: 3, editable: true }, 
        ...
        myDiagram.addDiagramListener('Modified', function (e) {
            ...
            if (currentFile === null)
                return;
            var text = currentFile.textContent || '';
            var idx = text.indexOf('*');
            if (myDiagram.isModified) {
                if (idx < 0)
                    currentFile.textContent = text + '*';
            }
            else {
                if (idx >= 0)
                    currentFile.textContent = text.substr(0, idx);
            }
        });
        // the Palette
    ...
    function rename(obj) {
        ...
        var newName = prompt('Rename ' + data.item + ' to:', data.item);
    ...
    function pasteSelection() { myDiagram.commandHandler.pasteSelection(myDiagram.toolManager.contextMenuTool.mouseDownPoint); }
