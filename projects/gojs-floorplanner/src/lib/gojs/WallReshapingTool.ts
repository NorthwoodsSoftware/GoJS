...
export class WallReshapingTool extends go.Tool {
   ...
   */
  constructor(init?: Partial<WallReshapingTool>) {
    super();
  ...
  set handle(value: go.GraphObject | null) {
    this._handle = value;
  }
  ...
  set adornedShape(value: go.Shape | null) {
  ...
  get returnPoint(): go.Point | null {
    return this._returnPoint;
  }
  set returnPoint(value: go.Point | null) {
  ...
  public updateAdornments(part: go.Part): void {
    ...
    if (part.isSelected && !this.diagram.isReadOnly) {
      ...
      if (seleltgo !== null && seleltgo.part !== null && seleltgo.part.data.category === 'Wall') {
  ...
  public canStart(): boolean {
    ...
    const h: go.GraphObject | null = this.findToolHandleAt(
  ...
  public doMouseMove(): void {
    ...
    if (tool.isActive && fp !== null) {
      ...
      if (fp.lastInput.shift) {
        ...
        const dx: number = stationaryPt.x;
      ...
      walls.iterator.each(function (w: go.Group) {
        if (w.data.key !== wall.data.key) {
          ...
          if (fpUtils.isPointInWall(w, mousePt)) {
  ...
  private getClosestIntersectingWall(proposedPt: go.Point): go.Group | null {
    ...
    const walls: go.Iterator<go.Group> = tool.diagram.findNodesByExample({
      category: 'Wall'
    }) as go.Iterator<go.Group>;
    let closestWall: go.Group | null = null;
    ...
    walls.iterator.each(function (w) {
      if (w.data.key !== wall.data.key && w.data.key !== dummyWall.data.key) {
        ...
        const ab: number = parseFloat(
          Math.sqrt(w.data.startpoint.distanceSquaredPoint(stationaryPt)).toFixed(2)
        );
        const bc: number = parseFloat(
          Math.sqrt(stationaryPt.distanceSquaredPoint(w.data.endpoint)).toFixed(2)
        );
        const ac: number = parseFloat(
          Math.sqrt(w.data.startpoint.distanceSquaredPoint(w.data.endpoint)).toFixed(2)
        );
  ...
  private performMitering(wa: go.Group, wb: go.Group): void {
    ...
    const wat: number = wa.data.thickness;
    const wbt: number = wb.data.thickness;
    const wal: number = Math.sqrt(as.distanceSquaredPoint(ae));
    const wbl: number = Math.sqrt(bs.distanceSquaredPoint(be));
    ...
    let ow: go.Group | null = null;
    let iw: go.Group | null = null;
  ...
  public getAllWallIntersectionPoints(): go.Set<string> {
    ...
    // get all walls
    const walls: go.Iterator<go.Group> = diagram.findNodesByExample({
      category: 'Wall'
    }) as go.Iterator<go.Group>;
    ...
    walls.iterator.each(function (w) {
      ...
      const otherWalls: go.Iterator<go.Group> = diagram.findNodesByExample({
        category: 'Wall'
      }) as go.Iterator<go.Group>;
   ...
   */
  public getAllWallsAtIntersection(
    intersectionPoint: go.Point | null,
  ...
  public maybeSplitWall(): void {
    ...
    const movingPt: go.Point =
      movingProp === 'sPt' ? reshapingWall.data.startpoint : reshapingWall.data.endpoint;
  ...
  private maybePerformWallSplit(w: go.Group, ip: go.Point): void {
    ...
    const text: string = w.data.isDivider ? 'Divider' : 'Wall';
    ...
    walls.iterator.each((wc: go.Group) => {
      const ws: go.Point = wc.data.startpoint;
      const we: go.Point = wc.data.endpoint;
      if (
        (this.pointsApproximatelyEqual(s, ws) && this.pointsApproximatelyEqual(ip, we)) ||
        (this.pointsApproximatelyEqual(s, we) && this.pointsApproximatelyEqual(ip, ws))
      ) {
        alreadyExists = true;
      }
      if (
        (this.pointsApproximatelyEqual(ip, ws) && this.pointsApproximatelyEqual(e, we)) ||
        (this.pointsApproximatelyEqual(ip, we) && this.pointsApproximatelyEqual(e, ws))
      ) {
        alreadyExists = true;
      }
    });
    ...
    if (l1 > 1 && l2 > 1 && !alreadyExists) {
      ...
      this.premiterWall(w1);
      this.premiterWall(w2);
      this.performMiteringAtPoint(ip, false);

      if (this.handle !== null) {
        const rooms: go.Iterator<go.Node> = fp.findNodesByExample({ category: 'Room' });
        const adorn: go.Adornment = this.handle.part as go.Adornment;
        ...
        rooms.iterator.each((r: go.Node) => {
          ...
          for (let i: number = 0; i < bw.length; i++) {
            ...
            if (wk === w.data.key) {
              ...
              const nonIpEndpoint: go.Point = this.pointsApproximatelyEqual(rw.data.startpoint, ip)
                ? rw.data.endpoint
                : rw.data.startpoint;
              const iw: go.List<go.Group> = this.getAllWallsAtIntersection(nonIpEndpoint);
              ...
              if (isConnectedToBounds) {
                ...
                const w1NonIpEndpoint: go.Point = this.pointsApproximatelyEqual(
                  w1.data.startpoint,
                  ip
                )
                  ? w1.data.endpoint
                  : w1.data.startpoint;
                const iw2: go.List<go.Group> = this.getAllWallsAtIntersection(w1NonIpEndpoint);
                ...
                iw2.iterator.each((ww: go.Group) => {
                  for (let j: number = 0; j < bw.length; j++) {
                    ...
                    if (ww.data.key === wk2 && w1.data.key !== ww.data.key) {
                      ...
                      const ww2OtherEndpoint: go.Point = this.pointsApproximatelyEqual(
                        ww2.data.startpoint,
                        w1NonIpEndpoint
                      )
                        ? ww2.data.endpoint
                        : ww2.data.startpoint;
                      const iw3: go.List<go.Group> =
                        this.getAllWallsAtIntersection(ww2OtherEndpoint);
                ...
                const replacementEntry = this.getUpdatedEntry(entry, replacementWall);
                ...
                const replacementEntry2: any = this.getUpdatedEntry(entry, wi.toArray()[1]);
                const replacementEntry1: any = this.getUpdatedEntry(entry, wi.toArray()[2]);
      ...
      wallsSet.add(w1);
      wallsSet.add(w2);
      this.maintainWallParts(wallParts, wallsSet);
      ...
      const w1op: go.Point = this.pointsApproximatelyEqual(w1.data.startpoint, ip)
        ? w1.data.endpoint
        : w1.data.startpoint;
      const w2op: go.Point = this.pointsApproximatelyEqual(w2.data.startpoint, ip)
        ? w2.data.endpoint
        : w2.data.startpoint;

      this.performMiteringAtPoint(ip, false);
      this.performMiteringAtPoint(w1op, false);
      this.performMiteringAtPoint(w2op, false);
    }
  }
  ...
  public isWallColinear(wa: go.Group, wb: go.Group): boolean {
    ...
    if (this.areWallsParallel(wa, wb)) {
      // get the endpoint shared by wa and wb, if it exists
      let sharedEndpoint: go.Point | null = null;
      if (this.pointsApproximatelyEqual(as, bs) || this.pointsApproximatelyEqual(as, be)) {
        sharedEndpoint = as;
      } else if (this.pointsApproximatelyEqual(ae, bs) || this.pointsApproximatelyEqual(ae, be)) {
        sharedEndpoint = ae;
      }

      if (sharedEndpoint !== null) {
        // Make sure all walls with an endpoint at sharedEndpoint are parallel to wa
        const wi: go.List<go.Group> = this.getAllWallsAtIntersection(sharedEndpoint);
        let endpointHasNonColinearWall: boolean = false;
        wi.iterator.each((w: go.Group) => {
          if (!this.areWallsParallel(w, wa)) {
  ...
  private findAllColinearWalls(w: go.Group, set?: go.Set<go.Group>): go.Set<go.Group> {
    ...
    walls.iterator.each((ow) => {
      if (this.isWallColinear(w, ow) && set !== undefined && !set.contains(ow)) {
        set.add(ow);
        this.findAllColinearWalls(ow, set);
      }
    });
    return set;
  }
  ...
  public joinColinearWalls(): void {
    ...
    const adorn: go.Adornment = this.handle.part as go.Adornment;
    const reshapingWall: go.Group = adorn.adornedPart as go.Group;
    const cw1: go.Set<go.Group> = this.findAllColinearWalls(reshapingWall);
    const jw: go.Group | null = this.performColinearWallJoining(cw1, reshapingWall);
    if (jw !== null) {
      this.joinedWalls.add(jw);
    }

    // 2) Check if there are 2 colinear walls at returnPoint (where the reshaping endpoint originally was)
    const wallsAtReturnPoint: go.List<go.Group> = this.getAllWallsAtIntersection(this.returnPoint);
    if (wallsAtReturnPoint.count === 2) {
      const wallsArr: Array<go.Group> = wallsAtReturnPoint.toArray();
      const w1: go.Group = wallsArr[0];
      const w2: go.Group = wallsArr[1];
      if (this.isWallColinear(w1, w2)) {
        const cw2: go.Set<go.Group> = new go.Set<go.Group>();
        cw2.add(w1);
        cw2.add(w2);
        const jw2: go.Group | null = this.performColinearWallJoining(cw2, w1);
        if (jw2 !== null) {
          this.joinedWalls.add(jw2);
        }
      }
    }
  }
  ...
  private performColinearWallJoining(
    ...
    if (colinearWalls.count > 1) {
      let pt1: go.Point | null = null;
      let pt2: go.Point | null = null;
      ...
      // make sure the mitering side is analogous to what it was with the constituent wall too
      const rooms: go.Iterator<go.Node> = fp.findNodesByExample({ category: 'Room' });
      rooms.iterator.each((r: go.Node) => {
        const bw: Array<any> = r.data.boundaryWalls;
        ...
        for (let i: number = 0; i < bw.length; i++) {
          ...
          if (colinearWalls.contains(ww)) {
            // ???
            this.performMiteringOnWall(newWall);

            const newEntry = this.getUpdatedEntry(e, newWall);
      ...
      this.performMiteringOnWall(newWall);
      this.maintainWallParts(wallParts, newWallSet);
  ...
  private maintainWallParts(wallParts: go.Iterable<go.Node>, walls: go.Iterable<go.Group>): void {
    ...
    wallParts.iterator.each(function (wp: go.Node) {
      ...
      walls.iterator.each(function (w: go.Group) {
        if (fpUtils.isPointInWall(w, loc)) {
  ...
  /**
   * Set a wall's mitering points in data to points thickness/2 distance from start/endpoint at perpendicular angle
  ...
  private premiterWall(w: go.Group, prop?: string): void {
    ...
    switch (prop) {
      ...
      default: {
        // Perpendicular mitering points (updated later during mitering function)
   ...
   * thickness/2 distance from start/endpoint at perpendicular angle
  ...
  /**
   * Perform corner mitering on all walls that intersect at a given point.
  ...
  public performMiteringAtPoint(pt: go.Point, performPremitering?: boolean): void {
    ...
    // sort all involved walls in any COUNTERCLOCKWISE order
  ...
  /**
   * Joins colinear walls, splits walls, performs mitering, omits transaction, stops tool.
   */
  ...
  public doDeactivate(): void {
    ...
    if (length < 1) {
      ...
      fp.pointNodes.iterator.each(function (node) {
        if (node.data.key.indexOf(wall.data.key) !== -1) wallDimensionLinkPointNodes.push(node);
      });
    ...
    if (diagram.lastInput.key === 'Escape' && !this.isBuilding) {
      ...
      if (this.returnData) {
        this.returnData.iterator.each(function (kvp: go.IKeyValuePair<string, go.Point>) {
    ...
    const glPoints: go.Iterator<go.Node> = this.diagram.findNodesByExample({
  ...
  public calcAngleAndLengthFromHandle(mousePt: go.Point): void {
    ...
    const distanceBetween: number = Math.sqrt(
  ...
  public snapPointToGrid(point: go.Point): go.Point {
    const diagram: go.Diagram = this.diagram;
    const gs: number = diagram.toolManager.draggingTool.isGridSnapEnabled
  ...
  public reshape(newPoint: go.Point): void {
    ...
    if (this.diagram.lastInput.shift && !this.isIntersecting) {
      ...
      if (angle > 22.5 && angle < 67.5) {
        let newx: number = Math.sin(0.785) * length;
        newx = gridSize * Math.round(newx / gridSize) + sPt.x;
        let newy: number = Math.cos(0.785) * length;
    ...
    const text: string = tool.handle.name;
    if (text === undefined) return;
    const stationaryPt: go.Point = text === 'sPt' ? node.data.endpoint : node.data.startpoint;
    const movingPt: go.Point = text === 'sPt' ? node.data.startpoint : node.data.endpoint;
  ...
  public showMatches(): void {
    ...
    const glPoints: go.Iterator<go.Node> = diagram.findNodesByExample({
      category: 'GLPointNode'
  ...
  /**
   * Checks if there exists a horizontal or vertical line (decided by 'coord' parameter) between pt and compare pt
   ...
   */
  public checkPtLinedUp(
    pt: go.Point,
    comparePtCoord: number,
    ptCoord: number,
    comparePt: go.Point
  ): void {
    function makeGuideLinePoint() {
      return new go.Node('Spot', {
        ...
        .add(new go.Shape({ stroke: null, strokeWidth: 1, name: 'SHAPE', fill: 'black' }));
    }

    function makeGuideLineLink() {
      return new go.Link().add(
        new go.Shape({ stroke: 'black', strokeWidth: 2, name: 'SHAPE' })
  ...
  private reshapeWall(
    wall: go.Group,
    stationaryPoint: go.Point,
    movingPoint: go.Point,
    newPoint: go.Point,
    diagram: go.Diagram
  ): void {
    const tool: WallReshapingTool = this;
    ...
    let closestPart: go.Part | null = null;
    let closestDistance: number = Number.MAX_VALUE;
    for (let i: number = 0; i < arr.length; i++) {
      const part: go.Part = arr[i];
      const distanceToStationaryPt: number = Math.sqrt(
      ...
      const distanceToMovingPt: number = Math.min(
    ...
    distancesMap.iterator.each(function (kvp) {
      ...
      const newLoc: go.Point = new go.Point(
        stationaryPoint.x + (distance / wallLength) * (movingPoint.x - stationaryPoint.x),
