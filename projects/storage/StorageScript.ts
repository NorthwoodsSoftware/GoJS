...
export function init() {
  ...
  (window as any).handlePromise = function(action) {
    ...
    switch (action) {
      ...
      case 'New': storageManager.create(myDiagram.isModified).then(function(fileData) {
        handleFileData(action, fileData);
      }); break;
  ...
  const defaultName = "Default.diagram";
  if (storageManager.currentStorage instanceof GoLocalStorage) {
    if ((window as any).localStorage["Default.diagram"]) {
      gls.load(defaultName).then(() => {
        updateTitle();
      });
    } else {
      // if not yet saved to localStorage, save the default model as Default.diagram, then load it
      gls.save(defaultName).then(() => {
        gls.load(defaultName).then(() => {
          updateTitle();
        });
      });
