...
export function init() {
    ...
    const openDocumentDiv = document.getElementById('openDocument');
    openDocumentDiv.style.visibility = 'hidden';
    // hides remove HTML Element
    const removeDocumentDiv = document.getElementById('removeDocument');
    ...
    go.Shape.defineFigureGenerator('Annotation', function (shape, w, h) {
        ...
        return new go.Geometry().add(new go.PathFigure(len, 0)
    ...
    const tooltiptemplate = $('ToolTip', $(go.TextBlock, { margin: 3, editable: true }, new go.Binding('text', '', function (data) {
    ...
    const boundaryEventItemTemplate = $(go.Panel, 'Spot', {
        ...
        fromMaxLinks: 1,
        toMaxLinks: 0,
    }, new go.Binding('portId', 'portId'), new go.Binding('alignment', 'alignmentIndex', nodeActivityBESpotConverter), $(go.Shape, 'Circle', { desiredSize: new go.Size(EventNodeSize, EventNodeSize) }, new go.Binding('strokeDashArray', 'eventDimension', function (s) {
        return s === 6 ? [4, 2] : null;
    ...
    const activityNodeMenu = $('ContextMenu', $('ContextMenuButton', $(go.TextBlock, 'Add Email Event', { margin: 3 }), {
        click: function (e, obj) {
    ...
    function makeMarkerPanel(sub, scale) {
        return $(go.Panel, 'Horizontal', { alignment: go.Spot.MiddleBottom, alignmentFocus: go.Spot.MiddleBottom }, $(go.Shape, 'BpmnActivityLoop', {
            width: 12 / scale,
            height: 12 / scale,
            margin: 2,
            visible: false,
    ...
    }), new go.Binding('stroke', 'eventDimension', nodeEventDimensionStrokeColorConverter), new go.Binding('strokeDashArray', 'eventDimension', function (s) {
    ...
    new go.Binding('visible', 'eventDimension', function (s) {
        ...
        desiredSize: new go.Size(EventNodeSymbolSize, EventNodeSymbolSize),
        stroke: 'black',
    }, new go.Binding('figure', 'eventType', nodeEventTypeConverter), new go.Binding('fill', 'eventDimension', nodeEventDimensionSymbolFillConverter))), // end Auto Panel
    ...
    // --------------------------------------------------------------------------------------------------------------
    const annotationNodeTemplate = $(go.Node, 'Auto', { background: GradientLightGray, locationSpot: go.Spot.Center }, new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, 'Annotation', // A left bracket shape
    {
        portId: '',
        fromLinkable: true,
        cursor: 'pointer',
    ...
    const dataObjectNodeTemplate = $(go.Node, 'Vertical', { locationObjectName: 'SHAPE', locationSpot: go.Spot.Center }, new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, 'File', {
        name: 'SHAPE',
        portId: '',
        fromLinkable: true,
        toLinkable: true,
    ...
    const dataStoreNodeTemplate = $(go.Node, 'Vertical', { locationObjectName: 'SHAPE', locationSpot: go.Spot.Center }, new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, 'Database', {
        name: 'SHAPE',
        portId: '',
        fromLinkable: true,
        toLinkable: true,
    ...
    const privateProcessNodeTemplate = $(go.Node, 'Auto', { layerName: 'Background', resizable: true, resizeObjectName: 'LANE' }, new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, 'Rectangle', { fill: null }), $(go.Panel, 'Table', // table with 2 cells to hold header and lane
    {
    ...
    const privateProcessNodeTemplateForPalette = $(go.Node, 'Vertical', { locationSpot: go.Spot.Center }, $(go.Shape, 'Process', {
        fill: DataFill,
    ...
    function assignGroupLayer(grp) {
        ...
        grp.layerName = lay;
        grp.findSubGraphParts().each(function (m) {
            m.layerName = lay;
        });
    }
    const subProcessGroupTemplate = $(go.Group, 'Spot', {
        ...
        memberValidation: function (group, part) {
            return ((!(part instanceof go.Group) && part.category !== 'Phase') ||
        ...
        mouseDrop: function (e, grp) {
            ...
            if (e.diagram.selection.all(part => part.category === 'Phase')) {
                ...
                while (pool && pool.category !== 'Pool')
                    pool = pool.containingGroup;
        ...
        return s ? ActivityNodeStrokeWidthIsCall : ActivityNodeStrokeWidth;
    })), $(go.Panel, 'Vertical', { defaultAlignment: go.Spot.Left }, $(go.TextBlock, // label
    { margin: 3, editable: true }, new go.Binding('text', 'text').makeTwoWay(), new go.Binding('alignment', 'isSubGraphExpanded', function (s) {
    ...
    const swimLanesGroupTemplate = $(go.Group, 'Spot', groupStyle(), {
        ...
        mouseDrop: function (e, grp) {
            ...
            if (!e.diagram.selection.any((n) => (n instanceof go.Group && n.category !== 'subprocess') ||
    ...
    $(go.Placeholder, { padding: 12, alignment: go.Spot.TopLeft, alignmentFocus: go.Spot.TopLeft }), $(go.Panel, 'Horizontal', { alignment: go.Spot.TopLeft, alignmentFocus: go.Spot.TopLeft }, $(go.TextBlock, // this TextBlock is only seen when the swimlane is collapsed
    {
        ...
        return !e;
    }).ofObject(), new go.Binding('text', 'text').makeTwoWay())), $(go.Shape, "LineH", { height: 0, alignment: go.Spot.Bottom, stretch: go.GraphObject.Horizontal })); // end swimLanesGroupTemplate
    // define a custom resize adornment that has two resize handles if the group is expanded
    // myDiagram.groupTemplate.resizeAdornmentTemplate =
    swimLanesGroupTemplate.resizeAdornmentTemplate = $(go.Adornment, 'Spot', $(go.Placeholder), $(go.Shape, // for changing the length of a lane
    {
    ...
    new go.Binding('text').makeTwoWay())), $(go.Placeholder, { column: 1 }))); // end poolGroupTemplate
    const phaseTemplate = $(go.Part, "Table", {
    ...
    }, new go.Binding("position", "pos", go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, { columnSpan: 2, name: "BACK", fill: 'rgba(0,200,0,0.05)', strokeWidth: 0 }, new go.Binding("width", "width").makeTwoWay()), $(go.TextBlock, {
        column: 0, alignment: go.Spot.Top,
        font: "bold 12pt sans-serif", editable: true
    }, new go.Binding("text").makeTwoWay()), $(go.RowColumnDefinition, { column: 1, sizing: go.RowColumnDefinition.None }), $(go.Shape, "LineV", {
    ...
    const sequenceLinkTemplate = $(go.Link, {
        ...
        }), $('ContextMenuButton', $(go.TextBlock, 'Conditional Flow'), 
        // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
        {
            click: function (e, obj) {
    ...
    myDiagram = new go.Diagram('myDiagramDiv', {
        ...
        mouseDrop: function (e) {
            if (e.diagram.selection.any(part => part.category === 'Phase'))
    ...
    myDiagram.addDiagramListener('LinkDrawn', function (e) {
        ...
        else if (e.subject.fromNode.category === 'dataobject' ||
            e.subject.toNode.category === 'dataobject') {
            e.subject.category = 'data'; // data association
        }
        else if (e.subject.fromNode.category === 'datastore' ||
...
class PoolLayout extends go.GridLayout {
    ...
    doLayout(coll) {
        ...
        if (pool !== null && pool.category === 'Pool') {
            const phases = new go.Set();
            const lanes = new go.Set();
            // make sure all of the Group Shapes are big enough
            const minsize = computeMinPoolSize(pool);
            pool.memberParts.each(function (lane) {
                if (lane.category === 'Phase')
                    phases.add(lane);
...
export function addActivityNodeBoundaryEvent(evType, evDim) {
...
export function loadFile() {
    const listbox = document.getElementById('mySavedFiles');
...
export function loadDiagramProperties(e) {
...
export function removeFile() {
    const listbox = document.getElementById('mySavedFiles2');
    if (listbox === null)
        return;
    // get selected filename
    let fileName;
...
export function updateFileList(id) {
...
export function openElement(id, listid) {
...
export function alignRows() {
    myDiagram.commandHandler.alignRow(askSpace());
}
export function alignColumns() {
    myDiagram.commandHandler.alignColumn(askSpace());
}
